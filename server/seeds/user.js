...
const generateBio = () => {
    const descriptions = bioDescriptors.sort(() => 0.5 - Math.random()).slice(0,3);
...
const sendUsers = async () => {
    try {
        ...
        const maleUser = maleNames.map((_, i) =>  generateRandomUser("male", i));
        const femaleUser = femaleNames.map((_, i) => generateRandomUser("female", i));

        const allUsers = [...maleUser, ...femaleUser];
